...
async function interpretSearchQuery(q) {
  ...
  try {
    ...
    if (STATE.provider === 'anthropic') {
      ...
      var _res = await fetch('https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent?key=' + STATE.apiKey, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
...
async function searchBooks() {
  var rawTitle = document.getElementById('book-search-title').value.trim();
  var rawAuthor = document.getElementById('book-search-author').value.trim();
  var raw = rawTitle || rawAuthor;
  if (!raw) return;
  ...
  // ISBN detection
  var isbnClean = raw.replace(/[-\s]/g, '');
  if (/^(97[89])?\d{9}[\dXx]$/.test(isbnClean)) {
    statusEl.textContent = 'Looking up ISBN…';
    statusEl.className = 'status-msg';
    statusEl.style.display = 'block';
    try {
      var isbnUrl = 'https://openlibrary.org/api/books?bibkeys=ISBN:' + isbnClean + '&format=json&jscmd=data';
      var res = await fetch(isbnUrl);
      var data = await res.json();
      var key = 'ISBN:' + isbnClean;
      if (data[key]) {
        var b = data[key];
        var book = {
  ...
  // Build search terms
  var searchTitle = rawTitle,
    searchAuthor = rawAuthor;
  statusEl.textContent = 'Searching…';
  statusEl.className = 'status-msg';
  statusEl.style.display = 'block';

  // AI natural language interpretation (title field only, no author given)
  var isNL = !rawAuthor && looksLikeNaturalLanguage(rawTitle);
  if (isNL && STATE.apiKey) {
    statusEl.textContent = 'Understanding your search…';
    var interp = await interpretSearchQuery(rawTitle);
  ...
  var gbQuery = gbParts.join('+');
  var olQuery = [searchTitle, searchAuthor].filter(Boolean).join(' ');
  var hasNonLatin = /[\u4e00-\u9fff\u3040-\u30ff\uac00-\ud7af\u0600-\u06ff\u0400-\u04ff\u0900-\u097f\u0e00-\u0e7f]/.test(raw);
  try {
    var _addShowMoreBtn = function addShowMoreBtn() {
      ...
      moreBtn.onclick = async function () {
        ...
        try {
          ...
          var fresh = more.filter(function (b) {
            var k = b.title.toLowerCase() + '||' + b.author.toLowerCase();
            if (_seen.has(k)) return false;
            _seen.add(k);
            return true;
          });
          var _manualEntry = resultsEl.querySelector('.manual-entry');
          moreBtn.remove();
          if (fresh.length) {
    ...
    if (!searchTitle && searchAuthor) {
      var refineEl = document.createElement('p');
    ...
    // Pagination state
    var _page = 1;
    var _olQuery = olQuery,
      _gbQuery = gbQuery,
      _hasNonLatin = hasNonLatin;
    var _seen = new Set(books.map(function (b) {
...
async function fetchOpenLibrary(q) {
  ...
  var res = await fetch('https://openlibrary.org/search.json?q=' + encodeURIComponent(q) + '&limit=' + limit + '&offset=' + offset + '&fields=key,title,author_name,first_publish_year');
  var data = await res.json();
  var seen = new Map();
  ...
  var results = Array.from(seen.values());
...
async function fetchGoogleBooks(q) {
  var maxResults = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 8;
  var startIndex = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 0;
  var res = await fetch('/api/books?q=' + encodeURIComponent(q) + '&maxResults=' + maxResults + '&startIndex=' + startIndex);
  var data = await res.json();
  var seen = new Map();
...
async function renderStatusScreen(book) {
  var lang = STATE.detectedLang;
  var chatLang = STATE.chatLanguage;
  var options = STATUS_OPTIONS_EN;

  // Translate if native language chosen and language detected
  if (chatLang === 'native' && lang && STATE.apiKey) {
    var cacheKey = 'pc_status_opts_' + lang.toLowerCase();
    ...
    if (cached) {
      ...
      try {
        ...
        if (STATE.provider === 'anthropic') {
          ...
          var _res3 = await fetch('https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent?key=' + STATE.apiKey, {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json'
            },
          ...
          var _res4 = await fetch('https://api.groq.com/openai/v1/chat/completions', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
              'Authorization': 'Bearer ' + STATE.apiKey
        ...
        var translated = JSON.parse(text.replace(/```json|```/g, '').trim());
  ...
  var container = document.querySelector('#screen-status .status-options');
  if (!container) return;
  container.innerHTML = '';
  ...
  // Update heading
  var h1 = document.getElementById('status-book-title');
...
async function selectBook(book) {
  ...
  var detectedLang = detectLanguage(book);
  if (detectedLang) book.detectedLang = detectedLang;
  localStorage.setItem('pc_last_book', JSON.stringify(book));
  var bk = bookKey(book);
  var savedStatus = localStorage.getItem('pc_status_' + bk);
  var savedLang = localStorage.getItem('pc_lang_' + bk);
  if (savedStatus) {
...
async function setReadingStatus(status) {
  ...
  var lang = detectLanguage(STATE.book);
...
async function setLanguage(choice) {
  ...
  var cacheKey = 'pc_icebreakers_' + bookKey(STATE.book) + '_' + (STATE.readingStatus || '');
...
async function generateThinkingPhrases(language) {
  var cacheKey = 'pc_thinking_' + language.toLowerCase();
  var cached = localStorage.getItem(cacheKey);
  if (cached) {
    try {
  ...
  try {
    ...
    if (STATE.provider === 'anthropic') {
      ...
      var _res5 = await fetch('https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent?key=' + STATE.apiKey, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
      ...
      var _res6 = await fetch('https://api.groq.com/openai/v1/chat/completions', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': 'Bearer ' + STATE.apiKey
    ...
    var phrases = JSON.parse(text.replace(/```json|```/g, '').trim());
...
async function populateIcebreakers(book) {
  var list = document.getElementById('icebreaker-list');
  list.innerHTML = '';
  ...
  var cacheKey = 'pc_icebreakers_' + bookKey(book) + '_' + (STATE.readingStatus || '');
  try {
  ...
  if (STATE.apiKey) {
    try {
      prompts = await fetchAIIcebreakers(book);
...
async function fetchAIIcebreakers(book) {
  ...
  var statusLabel = statusLabels[STATE.readingStatus] || 'reading';
  ...
  if (STATE.provider === 'anthropic') {
    ...
    var _res7 = await fetch('https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent?key=' + STATE.apiKey, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
    ...
    var _res8 = await fetch('https://api.groq.com/openai/v1/chat/completions', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': 'Bearer ' + STATE.apiKey
...
async function sendMessage(retryText) {
  ...
  var text = retryText || inputEl.value.trim();
  ...
  var loadEl = document.getElementById('loading-indicator');
