...
function applyLanguage(lang) {
  ...
  try { updateSearchHeading(); } catch (e) {}
  try { if (STATE.book) updateStatusDisplay(); } catch (e) {}
}
function setUILang(lang) {
  if (!UI_STRINGS[lang]) return;
  try { localStorage.setItem('pc_ui_lang', lang); } catch (e) {}
    ...
    return _regenerator().w(function (_context5) {
      while (1) switch (_context5.n) {
        ...
        case 2:
          ...
          results = Object.keys(seen).map(function (k) { return seen[k]; }); // Sort: exact title matches first, then by year
...
function fetchOpenLibraryISBN(isbn) {
  ...
  return fetch(url).then(function (r) {
    ...
    return {
      ...
      author: (b.authors || [{ name: t('js.unknown_author') }])[0].name,
    ...
    return _regenerator().w(function (_context6) {
      while (1) switch (_context6.n) {
        ...
        case 2:
          ...
          return _context6.a(2, seenOrder.map(function (k) { return seen[k]; }).slice(0, 6));
      }
    }, _callee6);
  }));
  return _fetchGoogleBooks.apply(this, arguments);
...
// ═══════════════════════════════════════════════════
var STOP_WORDS = { 'the': 1, 'a': 1, 'an': 1, 'of': 1, 'and': 1, 'in': 1, 'on': 1, 'at': 1, 'to': 1, 'for': 1, 'by': 1 };
...
function _icebreakerStrings(v) {
  ...
  for (var i = 0; i < v.length; i++) {
    ...
    else if (v[i] && typeof v[i].text === 'string' && v[i].text.trim()) out.push(v[i].text.trim());
  }
...
function parseIcebreakerList(text) {
  ...
  var arr = null;
  try { arr = _icebreakerStrings(JSON.parse(clean)); } catch (e) {}
  if (arr) return arr;
  // Extract the first [...] block from surrounding prose and parse that
  var m = clean.match(/\[[\s\S]*\]/);
  if (m) {
    try { arr = _icebreakerStrings(JSON.parse(m[0])); } catch (e2) {}
    if (arr) return arr;
  }
...
function appendBubble(role, text) {
  ...
  html = html.replace(/\[RECOMMEND:\s*([^\]]+)\]/g, function(match, q) {
    ...
    var jsEsc = function(s) { return s.replace(/\\/g, '\\\\').replace(/'/g, "\\'"); };
    var t = jsEsc(title);
    var a = jsEsc(author);
  ...
  if (role === 'companion' || role === 'user') {
    ...
    var alreadySaved = getPassages().some(function (p) { return p.text === text; });
...
function renderShelf() {
  ...
  function makeBookEl(book, isArchived) {
    ...
    var actionBtn = isArchived
      ? '<button class="shelf-archive-btn" data-action="unarchive">' + esc(t('js.restore')) + '</button>'
      : '<button class="shelf-archive-btn" data-action="archive">' + esc(t('js.archive')) + '</button>';
    ...
    el.innerHTML = '<div class="shelf-book-title">' + esc(book.title) + '</div>'
      ...
      + '<div class="shelf-book-meta">' + (statusLabel ? esc(statusLabel) + ' · ' : '') + esc(convCount) + (last ? ' · ' + esc(t('js.last')) + ': ' + last : '') + '</div>'
      + actionBtn;
...
function exportConversation(format) {
  ...
  if (STATE.currentConvId && book) {
    var convs = getConvs(book);
    var conv = convs.find(function(c) { return c.id === STATE.currentConvId; });
  ...
  var date = now.toLocaleDateString(dateLocale(), { day: 'numeric', month: 'long', year: 'numeric' });
...
function renderNotesPanel() {
  ...
  list.innerHTML = notes.map(function(n) {
    var d = new Date(n.ts).toLocaleDateString(dateLocale(), { day: 'numeric', month: 'short', year: 'numeric' });
...
function gdriveFindFileInFolder(folderId, filename) {
  ...
  return gdriveFetch('https://www.googleapis.com/drive/v3/files?q=' + encodeURIComponent(q) + '&fields=files(id)')
    .then(gdriveJson)
    .then(function(d) { return d && d.files && d.files.length ? d.files[0].id : null; });
}
...
function gdriveUploadMarkdown(folderId, existingId, filename, content) {
  ...
  var meta = existingId ? {} : { name: filename, parents: [folderId], mimeType: 'text/markdown' };
  var bodyStr =
    '--' + boundary + '\r\n' +
...
function syncConversationMarkdowns(convFolderId) {
  ...
  for (var li = 0; li < localStorage.length; li++) {
    ...
    var bk = k.slice(9);
    var convList = [];
    try { convList = JSON.parse(localStorage.getItem(k) || '[]'); } catch (e) {}
    ...
    for (var wi = 0; wi < withMsgs.length; wi++) {
      ...
      var dateStr = d.toLocaleDateString(dateLocale(), { day: 'numeric', month: 'long', year: 'numeric' });

      var fname = withMsgs.length > 1
        ? iso + '-' + titleSafe + '-' + (wi + 1) + '.md'
        : iso + '-' + titleSafe + '.md';
      ...
      tasks.push({ filename: fname, content: lines.join('\n') });
    }
  }
...
function importClippingsFromDrive() {
  ...
  if (msgEl) { msgEl.style.display = 'block'; msgEl.textContent = 'Looking for My Clippings.txt in your Page Commons folder…'; }
  getOrCreatePageCommonsFolder()
    .then(function(folderId) {
    ...
    .then(function(fileId) {
      ...
      return gdriveDownloadText(fileId).then(function(text) {
        ...
        for (i = 0; i < fresh.length; i++) {
          ...
          if (!seen[dk]) { existing.push(fresh[i]); seen[dk] = 1; }
        }
...
function showDriveClippingsBooksConfirm(highlights) {
  ...
  for (i = 0; i < highlights.length; i++) {
    ...
    if (!bookMap[k]) bookMap[k] = { title: h.title, author: h.author, count: 0 };
    bookMap[k].count++;
  }
  ...
  for (i = 0; i < keys.length; i++) {
    ...
    for (var j = 0; j < shelf.length; j++) {
      if (bookKey(shelf[j]) === bk) { onShelf = true; break; }
    }
    books.push({ title: entry.title, author: entry.author, count: entry.count, onShelf: onShelf });
  }
  ...
  var newCount = 0;
  for (i = 0; i < books.length; i++) { if (!books[i].onShelf) newCount++; }
  ...
  for (i = 0; i < books.length; i++) {
    ...
    if (b.onShelf) {
      html += '<div style="padding:10px 0;border-bottom:1px solid #eeeeee;color:#777777">'
        + et + ' <em>by</em> ' + ea + ' <span style="font-size:0.8rem">(already on shelf)</span></div>';
    } else {
      html += '<div style="padding:10px 0;border-bottom:1px solid #eeeeee">'
        + '<label style="display:-webkit-box;display:-webkit-flex;display:flex;-webkit-box-align:start;'
        + '-webkit-align-items:flex-start;align-items:flex-start;cursor:pointer">'
        + '<input type="checkbox" data-title="' + et + '" data-author="' + ea + '" checked '
        + 'style="margin-right:10px;margin-top:3px;-webkit-flex-shrink:0;flex-shrink:0">'
        + '<span>' + et + ' <em>by</em> ' + ea + countNote + '</span></label></div>';
    }
  }
  html += '</div>';
  if (newCount > 0) {
...
function confirmAddDriveBooks() {
  ...
  for (var i = 0; i < checkboxes.length; i++) {
    var cb = checkboxes[i];
    if (cb.checked) {
      addBookToShelf({ title: cb.getAttribute('data-title'), author: cb.getAttribute('data-author') });
      added++;
    }
  }
  if (confirmEl) {
    confirmEl.innerHTML = '<p style="margin-top:0">'
      + (added > 0 ? added + ' book' + (added !== 1 ? 's' : '') + ' added to your shelf.' : 'No books added.')
      + ' Your highlights are active — your companion will use them in chat.</p>';
  }
}
